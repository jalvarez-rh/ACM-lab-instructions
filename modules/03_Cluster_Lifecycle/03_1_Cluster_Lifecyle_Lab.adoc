...
In this lab environment, you have two OpenShift clusters provisioned and each one has a specific role.
...
In this lab, both clusters are deployed in a supported cloud provider.
...
Finally, both of these clusters are the same size.
...
While deploying a new cluster is outside the scope of this lab, it is fully supported and is a key feature of Cluster Lifecycle in RHACM.
As you can see below, you can use either a UI wizard or a YAML file when defining your deployment, or a combination of both.
...
You successfully imported an OpenShift cluster to be managed by RHACM from this point on.
You use this cluster as a target for application deployments and security policies in subsequent labs.
Later, in the _Visibility_ lab, you explore the usage of this newly _managed_ cluster.
